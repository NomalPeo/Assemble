...
<link rel="stylesheet" type="text/css"
	href="/resources/css/freeboard/freeboardCont.css">
<style type="text/css">
/*댓글 수정 화면 */
#modDiv {
	width: 300px;
	...
	padding: 10px;
	z-index: 1000;
	/* position속성이 absolute or fixed인 곳에서 사용한다. 이 속성은 요소가 겹쳐지는 순서를 제어할 수 있다. 물론,
   큰숫자일수록 앞에 나온다. */
}
</style>
</head>
<body>
	<div class="freeboard_cont">
		...
		<table border="1">
			...
			<tr>
				<td class="board_writer">${b.board_writer}&nbsp;| &nbsp;
					${b.board_date}</td>
		...
		<div class="board_btn_tr">
			<tr>
				<th class="board_btn" colspan="2"><sec:authorize
						access="hasAnyRole('ROLE_MEMBER','ROLE_ADMIN')">
						<input type="button" value="수정"
							onclick="location='freeboard_edit?board_no=${b.board_no}&page=${page}&board_writer=${b.board_writer }';" />
						<input type="button" value="삭제"
							onclick="location='freeboard_del?board_no=${b.board_no}&page=${page}&board_writer=${b.board_writer }';" />
					</sec:authorize> <input type="button" value="목록"
					onclick="location='freeboard_list?page=${page}';" /></th>
			</tr>
		</div>
		<br>
		<hr>
		...
		<div>
			<div class="cntreply">
				[댓글<span
					style="color: black; font-weight: bolder; font-size: 26px; border-radius: 15px; padding: 3px;">${board.replycnt}</span>]
				<br>
			</div>
			<div class="replycont">
				<textarea rows="5" cols="30" name="board_reply_cont"
					id="newReplyText" placeholder="댓글을 입력해주세요."></textarea>
			</div>
